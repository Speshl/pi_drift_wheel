...
def main():
    ...
    for name in evdev.list_devices():
        ffDevice = InputDevice(name)
        print(f"Name: {ffDevice.name} Device {ffDevice.path}") #Capabilities: {dev.capabilities(True,True)}
        if ecodes.EV_FF in ffDevice.capabilities():
            break
    ...
    ffTarget = random.randint(1000, 2000)
    lastEffectId = None
    startTime = time.time()
    updates = 0
    while True:
        ...
        if abs(finalState.channels[0] - ffTarget) < 10:
            force = 0
            ffTarget = random.randint(1000, 2000)
        elif finalState.channels[0] < ffTarget:
            force = abs(finalState.channels[0] - ffTarget) * .001 * -1
        elif finalState.channels[0] > ffTarget:
            force = abs(finalState.channels[0] - ffTarget) * .001
        ...
        if lastEffectId != None:
            ffDevice.erase_effect(lastEffectId)
        lastEffectId = ffDevice.upload_effect(effect)

        repeat_count = 1
        ffDevice.write(ecodes.EV_FF, lastEffectId, repeat_count)

        updates+=1
        difTime = time.time()-startTime
        print(f"Target: {ffTarget} Value: {finalState.channels[0]}, Force: {force}")
